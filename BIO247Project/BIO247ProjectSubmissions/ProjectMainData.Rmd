...
plot(x=ObsData$Var1, y=ObsData$AdjObs, xlab= "Chromosome", ylab=("Relative Frequency of SNP (AU)"), pch=19, col='black', type='p')
...
chr6overlaps <- data.frame(findOverlaps(chr6pos, chr6genes))

chr6potgenes <- c()
for (each in chr6overlaps$subjectHits){
  chr6potgenes <- c(chr6potgenes, chr6gene[each, 'Gene family'])
}

chr6potgenes

test <- data.frame('start'=chr6, 'var1'=1)
test2 <- data.frame('seq'=chr6gene$'Begin position in sequence'/1000000, 'var1' = 0.8)


plot(test$start, test$var1, pch=19)
points(test2$seq, test2$var1)
